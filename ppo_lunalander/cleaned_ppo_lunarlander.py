...
# Hyperparameters - carefully tuned for LunarLander
ENV_NAME = "LunarLander-v3"
GAMMA = 0.99
LAMBDA = 0.97
EPS_CLIP = 0.2
K_EPOCHS = 4  # Reduced to prevent overfitting
LR_ACTOR = 3e-4
LR_CRITIC = 3e-4  # Same as actor for balance
BATCH_SIZE = 64
...
class PPOAgent:
    ...
    def update(self, states, actions, logprobs, rewards, dones, values):
        ...
        Perform PPO update:
        1. Compute advantages A_t using GAE:
           δ_t = r_t + γ V(s_{t+1}) - V(s_t)
           A_t = Σ_{l=0}^∞ (γ λ)^l δ_{t+l}
        2. Compute returns: R_t = A_t + V(s_t)
        3. Optimize surrogate objective with clipping:
           r_t(θ) = π_θ(a_t|s_t) / π_{θ_old}(a_t|s_t)
           L^{clip}(θ) = E[ min(r_t A_t,
                                  clip(r_t,1-ε,1+ε) A_t ) ]
        4. Joint loss: L = -L^{clip} + 0.5 * MSE(V, R) - 0.01 * H(π)
        ...
        for epoch in range(K_EPOCHS):
            ...
            for start in range(0, batch_size, BATCH_SIZE):
                ...
                with torch.no_grad():
                    ...
                    # approx_kl is another common way, often used for early stopping
...
def evaluate_and_record(agent, env_name="LunarLander-v3", filename="ppo_video.mp4"):
    """
...
def train():
    ...
    while global_step < MAX_EPISODES * TIMESTEPS_PER_BATCH:
        # Collect trajectories frgitom the environment using the current policy
